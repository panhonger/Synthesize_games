...
(function (global) {
    ...
    var $mcStart = document.getElementById('mc_start'),//返回对拥有指定 ID 的第一个对象的引用
        $mcLevel = document.getElementById('mc_level'),
        $btnCollection = document.getElementsByTagName('button'),//返回带有指定标签名的对象的集合
        $btnStart = $btnCollection[0],
        $btnPri = $btnCollection[1],
        $btnMid = $btnCollection[2],
        $btnExp = $btnCollection[3],
        $gameTime = document.getElementById('mc_game_time'),//游戏时间
        $gameMine = document.getElementById('mc_game_mine'),//剩余雷数
        ...
        $win = document.getElementById('mc_gameover_win'),//游戏胜利
        $lose = document.getElementById('mc_gameover_lose'),//游戏失败
        $overTime = document.getElementById('mc_gameover_time'),//时间
        $overChoose = document.getElementById('mc_gameover_choose'),
        $replay = document.getElementById('mc_replay'),//重新游戏
        $reselect = document.getElementById('mc_reselect'),//选择难度
        upExPos = {
            'top': '-400px',
            'opacity': '0'//设置可见不可见，0则仅仅不可见
        },
        downExPos = {
            'bottom': '-400px',
            'opacity': '0'
    ...
    function coordOfGrid(ele) {
        var parNode = ele.parentNode;// 对象的形式返回指定节点的父节点
        return {
            x: parNode.eleIndex(),
            y: ele.eleIndex()
        }
